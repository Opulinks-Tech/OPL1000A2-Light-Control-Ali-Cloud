...
uint8_t light_ctrl_set_hsv(uint16_t hue, uint8_t saturation, uint8_t value, uint8_t fade);
uint8_t light_ctrl_set_ctb(uint32_t color_temperature, uint8_t brightness, uint8_t fade);
...
uint8_t light_ctrl_get_ctb(uint32_t *color_temperature, uint8_t *brightness);
...
void light_effect_set_speed(uint16_t speed_per);
