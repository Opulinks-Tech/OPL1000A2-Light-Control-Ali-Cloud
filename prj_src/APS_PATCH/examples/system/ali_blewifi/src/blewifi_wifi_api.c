...
extern osTimerId g_tAdaLedMPBlinkId;
...
SHM_DATA void BleWifi_Wifi_SendStatusInfo(uint16_t uwType)
{
...
static int BleWifi_Wifi_EventHandler_Start(wifi_event_id_t event_id, void *data, uint16_t length)
{
    ...
    if (g_led_mp_mode_flag)
    {
        scan_config.ssid = "ADA_TEST_WIFI_CHEK_2007";
...
static int BleWifi_Wifi_EventHandler_ScanComplete(wifi_event_id_t event_id, void *data, uint16_t length)
{
    ...
    scan_config.ssid = "ADA_TEST_WIFI_CHEK_2007";
    scan_config.scan_type = (wifi_scan_type_t)2;
    //scan the specified SSID "ADA_TEST_WIFI_CHEK_2007"
    if (g_led_mp_mode_flag)
    {
        ...
        if (isMatched)  //find the specified SSID
        {
            if (g_tAdaLedMPBlinkId != NULL)
            {
                ...
                osTimerStart(g_tAdaLedMPBlinkId,500);
            }
        }
