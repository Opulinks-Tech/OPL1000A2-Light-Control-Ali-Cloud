...
SHM_DATA int ota_http_retrieve_offset_server_version(httpclient_t *client, httpclient_data_t *client_data, int offset, int parse_hdr, uint16_t *uwfid)
{
    ...
    do
    {
        ...
        {
            LOG_E(TAG, "http response code = %d \r\n", response_code);
...
int ota_http_retrieve_get_server_version(char* get_url, char* buf, uint32_t len, uint16_t *uwfid)
{
    ...
    // skip 2nd boot agent,  0x00000000 ~ 0x00002FFF : 12 KB,
    ...
    // skip 2st OTA header,  0x00003FFF ~ 0x00004FFF : 4 KB
    ...
    LOG_I(TAG, "get server version successfully \n");
    return ret;

}

int ota_http_get_server_version(char* get_url, char* buf, uint32_t len, uint16_t *uwfid)
{
    int ret = HTTPCLIENT_ERROR_CONN;
    httpclient_data_t client_data = {0};
    char *header = NULL;
    ...
    httpclient_set_custom_header(&g_fota_httpclient, header);
    ...
    // Response body start
    ret = ota_http_retrieve_offset_server_version(&g_fota_httpclient, &client_data, 0x1000, 1, uwfid);
    if (ret < 0)
    {
        LOG_E(TAG, "http retrieve offset error, ret = %d \r\n", ret);
        return ret;
...
SHM_DATA int ota_download_by_http_get_server_version(char *param, uint16_t *uwfid)
{
    ...
    *uwfid = 0;
    ...
    ret = ota_http_get_server_version(get_url, buf, OTA_BUF_SIZE, uwfid);
    
 do_exit:
    LOG_I(TAG, "%s result = %d \r\n", __func__, (int)ret);
...
SHM_DATA int ota_download_by_http(char *param)
{
    ...
    memset(&g_fota_httpclient, 0, sizeof(g_fota_httpclient));
